<template>
  ...
  >
    <Icon v-if="markerProps.type !== 'both'" :icon="markerProps.iconName" height="16px" color="#fff"/>

    <div v-else class="flex relative align-center justify-center">
      <Icon icon="carbon:user-filled" height="14px" color="#fff" class="absolute left-[calc(50%-5px)] translate-x-[-50%]"/>
      <div>
        
      </div>
      <Icon icon="material-symbols:pets" height="14px" color="#fff" class="absolute left-[calc(50%+5px)] translate-x-[-50%]"/>
    </div>
